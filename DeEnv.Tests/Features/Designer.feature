Feature: The operator IDE (designs library + instance design selector)
  ...
  `fn render()` over a `Db { designs: set of Design }` meta-schema. A `Design` is a WHOLE app â€”
  ...
  `/instances/<id>` is ONLY a design SELECTOR â€” a `<select>` dropdown of the designs with the
  instance's current one pre-selected + an Apply button that records the chosen design on the
  instance AND deploys it. The instanceâ†”design link is an EXPLICIT reference: each instance stores a
...
# Feature scenarios have been split into the other Designer* .feature files for maintainability (see plan doc).
