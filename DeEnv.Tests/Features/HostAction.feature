Feature: Host-side actions — sys.create / sys.publish / sys.clone / sys.delete (server-side instance ops)
  ...
  existing instance. A design crosses the wire as the object's id (one member of the designer's
  `designs` set); instance ids are bare ints. All are server-only builtins (no client mutation), carried
  over the `hostAction` WS op and the per-instance IHostActions seam. Driven at the WS-handler seam
  (like the schema bridge) — no browser.
