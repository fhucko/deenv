...
public sealed class DesignerVocabularyTests
{
    ...
    // Guard: every FRAMEWORK-OWNED history type must deny client writes in the designer's own access section
    ...
    // so their client-facing write floor must deny every create/edit/delete. Reads may still be explicitly
    // gated; the invariant here is write immutability.
    private static readonly string[] FrameworkHistoryTypes = ["Commit", "Branch"]; // add here when a new one lands

    [Test]
    public async Task Every_framework_history_type_is_write_locked_in_the_designer_document()
    {
        var desc = AppParse.Parse(File.ReadAllText(DesignerApp));
        var rules = desc.Rules ?? [];
        var floor = new AccessFloor(rules, new ExecObject
        {
            Id = 1,
            TypeName = "User",
            Props = new Dictionary<string, IExecValue> { ["role"] = new ExecText { Value = "Admin" } },
        });
        foreach (var typeName in FrameworkHistoryTypes)
        {
            var obj = new ExecObject { Id = 1, TypeName = typeName, Props = [] };
            await Assert.That(floor.CanWrite("create", typeName, obj)).IsFalse();
            await Assert.That(floor.CanWrite("edit", typeName, obj)).IsFalse();
            await Assert.That(floor.CanWrite("delete", typeName, obj)).IsFalse();
        }
    }
